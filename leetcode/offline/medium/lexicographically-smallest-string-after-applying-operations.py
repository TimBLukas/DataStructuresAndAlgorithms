...
# in any order on `s`:
#
# 1. Add `a` to all odd indices of `s` (0-indexed). Digits post `9` are cycled
#    back to `0`. For example, if `s = "3456"` and `a = 5`, `s` becomes
#    `"3951"`.
#
# 2. Rotate `s` to the right by `b` positions. For example, if `s = "3456"`
...
# `b`. For example, `"0158"` is lexicographically smaller than `"0190"`
...
class TestLexicographicallySmallestStringAfterApplyingOperations(unittest.TestCase):
    def test_example_1(self) -> None:
        self.assertEqual(Solution().findLexSmallestString(s="5525", a=9, b=2), "2050")

    def test_example_2(self) -> None:
        self.assertEqual(Solution().findLexSmallestString(s="74", a=5, b=1), "24")

    def test_example_3(self) -> None:
        self.assertEqual(Solution().findLexSmallestString(s="0011", a=4, b=2), "0011")
