...
[x1, y1, x2, y2], where (x1, y1) is the coordinate of its bottom-left corner,
...
class Solution:
    def isRectangleOverlap(self, rec1: List[int], rec2: List[int]) -> bool:
        x_is_valid = (
            ...
            or (rec1[0] < rec2[2] and rec2[2] < rec1[2])
            or (rec2[0] <= rec1[0] and rec2[2] >= rec1[2])
        )
        y_is_valid = (
            (rec1[1] < rec2[1] and rec2[1] < rec1[3])
            or (rec1[1] < rec2[3] and rec2[3] < rec1[3])
            or (rec2[1] <= rec1[1] and rec2[3] >= rec1[3])
        )
